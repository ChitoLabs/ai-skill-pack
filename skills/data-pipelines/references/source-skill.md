...
> On first activation of this skill in a conversation: if the host runtime exposes a skill registry or skill-loading mechanism, inspect the available skill names and compare them against the `recommended_skills` field in this file's frontmatter. For any that are missing, mention them once and offer to install only after explicit user approval:
> ```
