...
```bash
bash references/resources/deploy.sh [path]
```
Run the preserved deploy helper only after explicit user approval, and adapt the script path to the installed skill or candidate workspace layout instead of assuming a specific agent home directory.
