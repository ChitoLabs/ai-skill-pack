...
In general, passing `url: my_path` to `form_with` tells the form where to make the request. The other option is to pass Active Model objects to the form, as you will learn [below](#creating-forms-with-model-objects). You can also use [URL helpers](https://guides.rubyonrails.org/routing.html#path-and-url-helpers).
...
TIP: If you're using password input fields, you might want to configure your application to prevent those parameters from being logged. You can learn about how in the [Securing Rails Applications](https://guides.rubyonrails.org/security.html#logging) guide.
...
If you have a model with a [composite primary key](https://guides.rubyonrails.org/active_record_composite_primary_keys.html), the form building syntax is the same with slightly different output.
...
value](https://guides.rubyonrails.org/action_controller_overview.html#composite-key-parameters) from the parameters and update the record as it would with a singular
primary key.
...
If you have a [singular resource](https://guides.rubyonrails.org/routing.html#singular-resources), you will need to call `resource` and `resolve` for it to work with `form_with`:
...
TIP: Declaring a resource has a number of side effects. See the [Rails Routing from the Outside In](https://guides.rubyonrails.org/routing.html#resource-routing-the-rails-default) guide for more information on setting up and using resources.

WARNING: When you're using [single-table inheritance](https://guides.rubyonrails.org/association_basics.html#single-table-inheritance-sti) with your models, you can't rely on record identification on a subclass if only their parent class is declared a resource. You will have to specify `:url`, and `:scope` (the model name) explicitly.
...
For more information on Rails' routing system and the associated conventions, please see the [Rails Routing from the Outside In](https://guides.rubyonrails.org/routing.html) guide.
...
If the file is an image that needs to be stored with a model (e.g. user's profile picture), there are a number of tasks to consider, like where to store the file (on Disk, Amazon S3, etc), resizing image files, and generating thumbnails, etc. [Active Storage](https://guides.rubyonrails.org/active_storage_overview.html) is designed to assist with these tasks.
...
parameters](https://guides.rubyonrails.org/action_controller_overview.html#strong-parameters) in the controller
