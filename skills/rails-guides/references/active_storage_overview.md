...
Active Storage attachments are Active Record associations behind the scenes, so you can use the usual [query methods](https://guides.rubyonrails.org/active_record_querying.html) to look up records for attachments that meet specific criteria.
...
available configuration options in [Configuring Rails Applications](https://guides.rubyonrails.org/configuring.html#configuring-active-storage).
...
helper](https://guides.rubyonrails.org/form_helpers.html#uploading-files) to automatically annotate the
