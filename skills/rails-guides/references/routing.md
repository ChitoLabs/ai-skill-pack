...
[Rack](https://guides.rubyonrails.org/rails_on_rack.html) application.) The router also generates path and URL
...
action generally maps to a specific [CRUD](https://guides.rubyonrails.org/active_record_basics.html#crud-reading-and-writing-data) operation on your data. A single entry in
...
Instead of specifying `:to` as a String like `'articles#index'`, which corresponds to the `index` method in the `ArticlesController` class, you can specify any [Rack application](https://guides.rubyonrails.org/rails_on_rack.html) as the endpoint for a matcher:
