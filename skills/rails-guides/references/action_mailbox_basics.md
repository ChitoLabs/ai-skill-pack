...
while [Action Mailer](https://guides.rubyonrails.org/action_mailer_basics.html) is for *sending* them.

The inbound emails are routed asynchronously using [Active
Job](https://guides.rubyonrails.org/active_job_basics.html) to one or several dedicated mailboxes. These emails
are turned into
[`InboundEmail`](https://api.rubyonrails.org/classes/ActionMailbox/InboundEmail.html)
records using [Active Record](https://guides.rubyonrails.org/active_record_basics.html), which are capable of
...
email via [Active Storage](https://guides.rubyonrails.org/active_storage_overview.html), and responsible data
...
[Rails router](https://guides.rubyonrails.org/routing.html) that dispatches URLs to controllers, routing in
