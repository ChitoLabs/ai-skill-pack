...
Read `packages/cli-core/src/commands/<name>/README.md` as secondary context only. Use those files to flag mocked or stubbed commands and to cross-check API endpoint claims in the audited skill's `references/recipes.md`. Do not propose copying command READMEs into this repository; they include internal implementation detail and would bloat the skill.
...
Route changes by maintenance value. Every path below is inside the `clerk-cli` skill under audit, not inside this one:

- **Core loop, mental model, and safety** stay in `SKILL.md`.
- **Per-command flag details, recipes, and edge cases** belong in the audited skill's `references/recipes.md` or a new reference file.
- **Agent-mode branches** belong in the audited skill's `references/agent-mode.md`; `SKILL.md` gets only a concise summary.
- **Auth, key resolution, and targeting** belong in the audited skill's `references/auth.md`.
