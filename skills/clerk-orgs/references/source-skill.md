...
|-----------|-------------|
| `roles-permissions.md` | Default + custom roles, System Permissions catalog, permission naming |
| `invitations.md` | Backend API for invitations + built-in UI |
| `enterprise-sso.md` | SAML/OIDC per-org, domain verification, correct field access |
| `nextjs-patterns.md` | Next.js adaptations specific to orgs. For generic Next.js patterns see `clerk-nextjs-patterns` skill. |
...
- Roles + permissions catalog is editable in `roles-permissions.md`. Custom role creation goes through `clerk config patch` (instance-level role definitions) — see Dashboard's role editor for the UX equivalent.
- For SSO / verified domain provisioning, see `enterprise-sso.md`.
...
**Permission naming convention.** System Permissions prefix with `org:sys_`; custom Permissions use `org:<resource>:<action>`. The full System Permissions catalog lives in `roles-permissions.md` — the short list is:
...
Do NOT invent names like `org:create`, `org:manage_members`, `org:update_metadata` — those are not real permission slugs. See `roles-permissions.md` for custom roles and the permission table.
...
You can create up to 10 custom roles per instance in Dashboard → Organizations → Roles & Permissions. Role-per-org is controlled via **Role Sets** — see `roles-permissions.md` for the full model (custom roles, Creator/Default role settings, role sets, and the System Permissions catalog).
...
For middleware-level protection (Next.js) see `nextjs-patterns.md`.
...
The full lifecycle (list, revoke, bulk create, built-in `<OrganizationProfile />` UI) lives in `invitations.md`.
