...
Generate a unique deployment name from the account, resource group, and model name. The upstream skill calls `scripts/generate_deployment_name.sh` for this but does not publish that script, so build the name yourself:
