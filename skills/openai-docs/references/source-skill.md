...
1. Suggest the install command to the user: `codex mcp add openaiDeveloperDocs --url https://developers.openai.com/mcp`
2. If installation may require permissions/sandbox escalation, explain why escalation is needed and ask the user for explicit approval before retrying with escalated permissions.
3. Only run the install or escalated retry after the user approves; otherwise provide the command as a manual fallback.
