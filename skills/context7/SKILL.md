...
This skill enables retrieval of current documentation for software libraries and components by using the host runtime's Context7 documentation tools when available, or by querying the Context7 API with an available shell tool when direct tools are not available. Use it instead of relying on potentially outdated training data.
