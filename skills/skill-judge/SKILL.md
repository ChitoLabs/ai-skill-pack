...
Use this skill when the user request matches `skill-judge` or the preserved source description: Evaluate Agent Skill design quality against official specifications and best practices. Use when reviewing, auditing, or improving SKILL.md files and skill packages. Provides multi-dimensional scoring and actionable improvement suggestions.

Its score is advice, never a verdict. When the repository has its own skill policy or review, that policy decides publication and this rubric only informs it. Report the score and the findings; do not treat a number as approval or as a block.
