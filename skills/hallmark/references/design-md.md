...
Loaded by [`SKILL.md`](../SKILL.md) ONLY when the user explicitly asks Hallmark to lock the current build's design system into a portable file. The standard build does NOT auto-emit `design.md`. The user iterates freely until they say the system is settled, then asks for it.
...
For everything else  -  including a standard build, a single-page redesign, and free iteration on the same brief  -  skip. Single-page redesign and standard build stay token-portable via `tokens.css`; `design.md` is the explicit lock-in step.
...
| | **Build path** (lock the system) | **Study path** (lock the DNA) |
| --- | --- | --- |
...
- **Multi-page redesign keeps existing behaviour.** A multi-page redesign produces the heavyweight `design.md` per the [`workflows/redesign.md`](workflows/redesign.md) multi-page flow. That flow already implies a locked system, so the rule there is unchanged.
...
## CTA  -  surface the offer during Preview and Approval
...
Once `design.md` exists, [build workflow: Preflight and Design Authority](workflows/build.md#phase-1-preflight-and-design-authority) detects it on every subsequent build. From that point on:
