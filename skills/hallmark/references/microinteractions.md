...
```css
.copy-btn[data-state="copied"] .copy-btn__label::after { content: "  OK  Copied"; }
...
These are the microinteraction signatures of generated code. [`slop-test.md`](slop-test.md) checks for them; treat any one as a critical finding.
...
| --- | --- | --- | --- |
| Specimen | 1.0x | `--ease-out` | Default. Restrained. |
| Midnight | 0.9x | `--ease-out` | Snappy, technical. |
| Brutal | 0.75x | `--ease-out` (sharper) | Fast, decisive. No spring. |
| Garden | 1.2x | `--ease-out` | Calm. Springs welcome. |
| Atelier | 1.3x | `--ease-out` (very gentle) | Generous; almost no movement. |
| Newsprint | 0x | none | Static. Print metaphor. |
| Terminal | 0x | none, except caret blink *inside* a typed command (N8 nav, install code) | Print + monospace metaphor. **No standalone blinking cursor**  -  see the Caret blink row above. The caret only blinks where the user would type. |
| Manifesto | 0.7x | `--ease-out` (sharp) | Snap into place. |
| Almanac | 0.85x | `--ease-out` | Functional, like a reference book. |
| Sport | 0.7x | `--ease-out` (sharp) | Quick, italic-energy. |

If the theme has duration scale `0x`, you do not animate. The page does not move. That is a deliberate design choice; it is not broken.
...
- **Hit targets** >= 44 x 44 CSS px on touch surfaces.
