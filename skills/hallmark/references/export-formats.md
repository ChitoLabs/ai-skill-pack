...
Loaded by [build workflow: Stamp, Export, Persistence, and Handoff](workflows/build.md#phase-10-stamp-export-persistence-and-handoff) when the user approves portable design-system export. Defines the four canonical formats Hallmark writes for that branch:
...
The output rule: `tokens.css` is always written. The other three live inline in `design.md` so the user copies whichever they need into a new project. This is part of every applicable build.
...
When Phase 10 emits approved exports:
