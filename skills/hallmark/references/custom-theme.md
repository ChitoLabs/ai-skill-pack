...
Load only when [build workflow: Genre, Theme Route, and Diversification](workflows/build.md#phase-3-genre-theme-route-and-diversification) selects custom. Custom is **made-to-measure for one brief**, written inline into the page's `:root`, never a permanent catalog entry. It spans a **spectrum of depth**: at its lightest, a complete OKLCH palette + free-font pairing tuned to the brief while keeping Hallmark's structures; at its fullest  -  **bespoke**  -  the page's structure and composition are designed from first principles too. One route, chosen depth.

**The freedom is the combination  -  and, at the bespoke depth, the whole structure  -  but never the floor.** Every constraint in [`color.md`](color.md), [`typography.md`](typography.md), and [`anti-patterns.md`](anti-patterns.md) still applies. Every applicable slop-test gate and scoped recipe-exemption condition remains authoritative. [Build workflow: Preview and Approval](workflows/build.md#phase-7-preview-and-approval) surfaces the palette and pairing before code.
...
**Do not ask anything else.** Audience, use, tone, and the brand vibe from [Context and Scope](workflows/build.md#phase-2-context-and-scope) are enough signal. Do not ask the user to nominate paper lightness or font weights.
...
- one orchestrated motion; the Phase 7 preview before code; the Phase 10 stamp and log.
...
A custom theme must declare its three diversification-axis values explicitly so [build workflow: Genre, Theme Route, and Diversification](workflows/build.md#phase-3-genre-theme-route-and-diversification) applies the same rule as catalog themes.
...
The CSS comment at the top of the produced stylesheet follows [build workflow: Stamp, Export, Persistence, and Handoff](workflows/build.md#phase-10-stamp-export-persistence-and-handoff):
...
Catalog entries continue to record `theme: <name>` and skip `theme_axes`; custom entries record their axes. The Phase 3 diversification check reads catalog axes from bundled theme guidance and custom axes from the stored entry.
...
2. **Does not save themes for reuse.** A custom run is per-output. The portable candidate does not write back to a bundled token file. If the user wants a permanent theme, they paste the custom palette into their own tokens file and name it.
3. **Does not ask multiple follow-up questions.** One vibe answer plus an optional anchor is enough. Audience, use, tone, brief, and macrostructure already provide the needed signal.
