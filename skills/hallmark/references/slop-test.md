...
For a build, run this list after implementation as defined by [`workflows/build.md`](workflows/build.md) Phase 9. For an audit, use applicable gates as a read-only rubric without entering the build workflow. Every answer must be **no** unless one exact scoped recipe exemption below is active and all of its conditions pass.
...
## Scoped recipe exemptions

An exemption is valid only when its identifier is recorded in the build state and final stamp, the named recipe was explicitly selected, and every condition below is verified. No identifier may excuse a generic occurrence of the pattern.

- `recipe-exception: h8-real-capture`: Gate 47 permits H8 or E3 only when browser chrome is part of a real captured screenshot. Hand-built URL pills, traffic-light dots, toolbars, and window controls remain forbidden.
- `recipe-exception: specimen-left-margin`: Gate 54 permits S1 or Specimen left-margin numbering only when the selected macrostructure is explicitly editorial, type-foundry, journal, or specimen work; the labels carry real sequence or document meaning; and the pattern is not reused as generic SaaS section decoration.
- `recipe-exception: quote-led-italic`: Gate 38a permits italic display only on a real attributed `blockquote` in Quote-Led, H3, or T1. It does not permit italic `h1` through `h6`, fabricated testimony, or an isolated italic emphasis word inside a heading.
- `recipe-exception: letter-salutation-italic`: Gate 38a permits italic only for the salutation or sign-off in the Letter or Ft6 recipe. The main page heading remains roman.
- `recipe-exception: stat-led-italic`: Gate 38a permits an italic numeric figure in Stat-Led or H4 only when it displays a real user-supplied metric and is not the sole headline. Labels and headings remain roman.
- `recipe-exception: carnival-single-line-marquee`: Gate 55 permits line-height below 1.0 only for a single-line, non-wrapping Carnival hero word or marquee. It must use `white-space: nowrap`, pass rendered overflow checks at every required viewport, and never apply to a multi-line heading.
...
38a. Is any **heading or display type italic** (`font-style: italic` on `h1`-`h6`, a `.*__title`, `.hero__title`, a wordmark, a stat figure, a footer statement, or an `<em>`/`<i>` inside a heading)? If yes, fail unless the exact occurrence passes `quote-led-italic`, `letter-salutation-italic`, or `stat-led-italic`. Generic italic headings and isolated italic emphasis words inside upright headlines always fail. Headers are otherwise roman; emphasis comes from weight, accent colour, or a drawn underline.
...
The Phase 10 stamp records `* contrast: pass (40-41)` if both gates pass, or `* contrast: FAIL gates <list>` if any are open. Fix before shipping.
...
The Phase 10 stamp records the result alongside contrast: `* nav: N# * footer: Ft# * slop: pass (42-45)`. If any of 42-45 fail, fix before shipping.
...
48. **Mid-render token improvisation.** Did Hallmark introduce any colour value (`#hex`, `oklch(...)`, `rgb(...)`, `hsl(...)`) or `font-family` declaration *outside* the design tokens defined in `:root` / `[data-theme="..."]`? If yes, fail. Every colour and every font in the artifact must reference a named token. Lift any new value into the token block first. *(See [build workflow: Locked Tokens](workflows/build.md#phase-5-locked-tokens) and [anti-patterns.md § Mid-render token improvisation](anti-patterns.md).)*
...
The Phase 10 stamp records `* honest: pass (46) * chrome: pass (47) * tokens: pass (48) * responsive: pass (49) * icons: pass (30)`. Any failure must be fixed before shipping.
...
54. **Section eyebrow / tag beside the heading (tag-left, header-right).** Does any section render an eyebrow, number, or label beside its heading on the same horizontal row? Fail unless `specimen-left-margin` is active and every condition in that exemption passes. Without that exact exemption, the heading goes directly underneath any eyebrow in one column. Generic SaaS section numbering, structural-parity requests, and prior builds never qualify.

55. **All-caps display heads with line-height < 1.0 -> cap-collision on wrap.** Does any display-size element declare both uppercase transformation and line-height below 1.0? Fail unless `carnival-single-line-marquee` is active and its rendered non-wrapping conditions pass. Every multi-line all-caps display keeps a 1.0 floor, with 1.02 to 1.08 preferred.
...
57. **Studied DNA discarded for a catalog theme.** Did a study diagnosis emit earlier in the conversation, and does the build stamp name a catalog theme rather than `studied-DNA (source: ...)` without an explicit user pivot? Auto-fail. [Build workflow: Preflight and Design Authority](workflows/build.md#phase-1-preflight-and-design-authority) makes studied DNA the system. Re-emit using its tokens, macrostructure, and archetypes.

The Phase 10 stamp records mobile status alongside contrast: `* mobile: pass (34, 49, 50-57)` or `* mobile: BLOCKED` when rendered validation is blocked.
