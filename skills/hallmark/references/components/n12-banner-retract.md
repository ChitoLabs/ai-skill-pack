...
```
*JS:* track scroll direction  -  past ~48px going down -> `.is-compact`; going up -> remove it; near top -> always show. Dismiss x sets `--banner-h: 0px` (via `documentElement.style`) and adds `.is-dismissed`.
