...
The Lottie Tell, version 2026: a generic LottieFiles pull where pure CSS would have built it stronger and lighter. The audit workflow catches this.
...
**Macrostructure:** Long Document  -  the bakery is a story-led brand, not a SaaS product.
**Theme:** Atelier  -  warm-paper, prose-led, intimate.
**Enrichment:** E5 Custom Illustration Centerpiece. Tier B (hand-built SVG).
...
A small SVG character  -  face only, ~120 x 120 px  -  that has personality without anthropomorphic uncanny-valley risk. Two ellipse eyes (with `@keyframes blink` 3s loop), a single quadratic-curve mouth, and two stem accents (hair / hat / horns / antennae). Pairs beside text.
...
A small (~40 x 80 px) hand-drawn sprig with two asymmetric leaves at +25° and -30° rotations. Leaf veins at 0.6 opacity. Sized to sit beside a headline as an inline accent. Pure SVG, no animation by default (the design is the stillness).
