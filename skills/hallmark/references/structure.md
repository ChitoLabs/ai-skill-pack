...
| --- | --- | --- |
| **Left-margin** | **Opt-in only, never default.** A generic eyebrow or number beside a heading remains forbidden. The paired left-margin pattern is permitted only when the selected S1 or Specimen recipe explicitly activates `recipe-exception: specimen-left-margin`, the selected macrostructure is explicitly editorial, type-foundry, journal, or specimen work, the label carries real sequence or document meaning, and the pattern is not reused as generic SaaS section decoration. No other route or user request activates this exception. | The New York Times Magazine; our Specimen theme, only through the selected scoped recipe. |
...
| **Numbered display** | **Opt-in only, never default.** A side-by-side number beside a heading is allowed only under the same selected `specimen-left-margin` recipe exception and all of its constraints. For all other genuine ordinal content, stack the number above the heading. Generic SaaS, consumer, and developer-tool section numbering remains forbidden by slop-test gate 54. | Rauno Freiberg's portfolio, only when the exact selected recipe or stacked ordinal treatment applies. |
...
- **Outlined.** Border, no fill. Secondary or quiet primary. Hallmark's standard treatment.
...
A fingerprint = one choice per axis. There are 8 x 7 x 5 x 5 x 5 x 6 = **42 000** fingerprints. You will never run out.
