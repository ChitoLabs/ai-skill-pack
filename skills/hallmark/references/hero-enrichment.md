...
Load this file only from [build workflow: Enrichment Decision](workflows/build.md#phase-6-enrichment-decision), after macrostructure selection. It determines whether the hero needs media and, if so, which archetype to build.
...
**Knobs:**
- Frame style (real captured chrome with `h8-real-capture` * minimal hairline * none)
- Tilt angle (0° * 1.5° * 3°)
...
Same composition as E3 but without captured browser chrome  -  the screenshot floats with a soft shadow and 12 px corner radius. Cleaner; demands a higher-quality screenshot.
...
**Example.** A code-formatting CLI marketing page. Headline left ("Format anything, in eight lines."), a single floating screenshot right showing `before` / `after` code side by side. 12 px corner radius, a soft 24 px shadow at -10 px offset, sitting on a barely-tinted gradient surface. **No browser chrome**  -  the screenshot itself is composed and beautiful enough to stand naked. Use E3 only when a real capture already includes useful chrome.
...
.hero__num { position: absolute; right: var(--page-gutter); bottom: -0.15em; font-family: var(--font-display); font-style: normal; font-weight: 600; font-size: clamp(8rem, 22vw, 18rem); line-height: 1; color: color-mix(in oklch, var(--color-ink) 8%, transparent); pointer-events: none; user-select: none; }
...
- **Asymmetric padding.** `padding-block-end` >= 1.3x `padding-block-start`. The hero sits *into* the page; symmetric padding floats. Slop-test gate 44 enforces this.
...
The slop test ([`SKILL.md`](../SKILL.md)) carries four binary gates that mirror these questions; the audit workflow runs them.
...
This signals to future Hallmark runs and the audit workflow what was chosen and how. It also lets the user see the inferences in one place and redirect if anything's off.
