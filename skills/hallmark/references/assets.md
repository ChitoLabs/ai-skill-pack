...
| **[Tabler Icons](https://tabler-icons.io)** | `tabler-icons.io` | 5,900+ on a 24x24 grid | Breadth  -  when neither Lucide nor Heroicons covers the symbol you need. |
| **[Iconoir](https://iconoir.com)** | `iconoir.com` | ~1,500 | Hand-drawn character with a generous free tier. |

### The rules

1. **Pick one library per project.** Mixing Material + Heroicons + Lucide on the same page is the icon-set tell. The audit workflow catches this.
...
3. **2-3x height as gutter.** Logos need breathing room. A wall with 32-px logos wants 64-96-px gutters.
...
1. **Real capture for SaaS / web apps.** Communicates "this is real, on the web". Use a real screenshot that already contains browser chrome, or a plain 1 px figure boundary. Never hand-build URL pills, traffic-light dots, or window controls. Use `recipe-exception: h8-real-capture` only for captured chrome.
