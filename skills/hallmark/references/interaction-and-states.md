...
Minimum 44x44 CSS px for any touch-reachable element. Use padding or an `::before` overlay to expand the hit target without changing visual size:
...
| **Success** | Subtle accent-coloured border (3 % chroma above default) * small OK glyph * auto-clear if user re-edits | Quiet; success doesn't deserve celebration unless it was hard. |
