...
```
watercolor-warm-01.webp     1600x900   warm orange * cream paper      hero half-flood, atmospheric
watercolor-cool-01.webp     1600x900   cool blue * greyscale          quote-behind wash
transparent-brush-warm-01   1200x1200  burnt orange brushstroke       layered hero, off-centre
transparent-blob-cool-01    1200x1200  cobalt organic shape           layered hero, behind headline
ornament-stamp-01.svg       inline     ink-only stamp w/ numeral      label-gutter, letter close
texture-grain-paper-01      512x512    cream paper grain (tileable)   body grain, blend-multiply
silhouette-bottle-01.png    600x900    abstract bottle, transparent   empty product slot
silhouette-card-01.png      900x600    abstract product card          comparison row
pattern-cross-warm-01       400x400    cross-hatch warm (tileable)    section-band texture
```

Pick an asset during [build workflow: Enrichment Decision](workflows/build.md#phase-6-enrichment-decision) by matching the active theme's palette family and the brief's tone. Inspect the manifest, choose the closest match, and fall back if the file is unavailable.
...
```html
<!-- TODO: Replace with real product photo, target size: 600x900 -->
...
```
site/public/imagery/
  ├-- watercolor/        ~6 files x 4 palette families = 24 WebPs
  ├-- transparent/       ~6 files x 4 palette families = 24 PNGs
