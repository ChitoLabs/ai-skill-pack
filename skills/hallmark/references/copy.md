...
The skill bends toward distribution-default copy ("Built for the modern team", "Unleash your X", "Where A meets B") unless given non-default voices to imitate. The samples below are *real opening lines* from sites that defy that distribution. **Imitate the kind of specificity**  -  named places, named dates, named verticals, refusal of metaphor, refusal of the verb  -  not the wording. The tone column maps to the tones committed in [build workflow: Context and Scope](workflows/build.md#phase-2-context-and-scope).
