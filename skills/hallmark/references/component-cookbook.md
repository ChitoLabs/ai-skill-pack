...
- **H8 * Mockup split real-capture framed**  -  Headline left, real product capture right, tilted 1-3° for life. Captured chrome is allowed; reconstructed chrome is not. [`components/h8-mockup-split-browser-framed.md`](components/h8-mockup-split-browser-framed.md)
...
- **F1 * Bento grid**  -  Asymmetric grid of 8-15 tiles in mixed spans (1x1, 2x1, 1x2, 2x2). Visual rhythm via size. [`components/f1-bento-grid.md`](components/f1-bento-grid.md)
...
- **N12 * Announcement banner + retracting nav**  -  A coloured promo banner stacked above one real nav; banner retracts on scroll-down, returns on scroll-up, dismisses via x. Banner ≠ second nav (colour contrast keeps them distinct). [`components/n12-banner-retract.md`](components/n12-banner-retract.md)
...
| **H3 Quote-Led** | Quote weight: italic display (`quote-led-italic`) * roman display * roman body large | Attribution position: under quote * margin-aligned * right-flush | Length: <=80 chars * 80-160 chars |
| **H4 Stat-Led** | Number style: tabular display * italic display (`stat-led-italic`) * monospace | Qualifier position: below * inline-right * stacked-above | Secondary stats: none * two below * row of four |
...
| **H7 Demo Video Clipped-Edge** | Clip side: right * left * both | Aspect ratio: 16/10 * 16/9 * 4/3 | Frame: hairline * captured chrome (`h8-real-capture`) * none |
| **H8 Mockup Split** | Frame style: captured chrome (`h8-real-capture`) * minimal hairline * floating no-frame | Tilt: 0° * 1.5° * 3° | Screenshot count: 1 * stack-of-3 * orbit-of-3 |
...
| **T1 Pull quote w/ marginalia** | Quote treatment: italic display (`quote-led-italic`) * roman large * serif italic (`quote-led-italic`) | Attribution: signed * stamped * timestamped | Marginalia: none * timeline * 1 footnote |
| **T2 Logo wall (hairline)** | Layout: single row * 2 rows * grid 3xN | Logo treatment: monochrome ink * brand colour * ghosted | Divider: hairline cells * none |
...
| **T3 Single huge quote** | quote remains full width; attribution wraps below | quote size step down by 1.4x |
...
- All hit targets >= 44 x 44 px below 40 rem (WCAG AA). Never below.
