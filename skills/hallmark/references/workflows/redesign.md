...
Pages that drift from `design.md` are slop. The audit workflow flags `design.md` drift as a critical structural finding (`stamp-vs-design.md disagreement`).
...
If the user specifies a mood such as luxury, pick a tone aligned to that mood and let it drive the structural fingerprint. Mood names map to tones from [`typography.md`](../typography.md) and [`structure.md`](../structure.md). If no mood is given, ask the user what *feeling* they want  -  one word  -  and proceed.
