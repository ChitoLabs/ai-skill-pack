...
If browser or rendering capability is available and approved, inspect representative rendered viewports and label those findings as rendered evidence. If rendering is unavailable or denied, run source-only checks, label rendered validation `BLOCKED`, list unverified viewports and visual risks, and never claim responsive or full slop-test passage from source alone.
