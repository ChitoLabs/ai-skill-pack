...
Load during [build workflow: Genre, Theme Route, and Diversification](../workflows/build.md#phase-3-genre-theme-route-and-diversification) whenever the catalog pick is `hum`. The palette and font stack come from the source project's theme token catalog under `[data-theme="hum"]`. This file carries signature moves, motion direction, voice fixtures, and anti-patterns.
...
- **Social proof**  -  quote-cards-x3 is *one* option. Also: a single big pull-quote; a marquee of names; an inline stat+quote; a screenshot of real usage. The byline grammar "Name * metric" on every card is itself a tell  -  vary it.
