...
Load during [build workflow: Genre, Theme Route, and Diversification](../workflows/build.md#phase-3-genre-theme-route-and-diversification) whenever the catalog pick is `lumen`. Palettes and font stack come from the source project's theme token catalog under `[data-theme="lumen"]` (Night) and `[data-theme="lumen"][data-drop="day"]` (Day). This file carries drops, apparatus family, signature moves, motion, and anti-patterns.
