...
Load during [build workflow: Genre, Theme Route, and Diversification](../workflows/build.md#phase-3-genre-theme-route-and-diversification) whenever the catalog pick is `carnival`. The default palette ("Cold Snap") and font stack come from the source project's theme token catalog under `[data-theme="carnival"]`; this file carries the palette drops plus rules tokens cannot encode.
...
2. **Decorative ornaments.** Use `✱` (asterisk operator), `❋` (heavy six-petalled), `◆` (black diamond) as bullets, section dividers, and rhythmic spacers. Patterns:
   - Section dividers: `✱ ✱ ✱ ✱` (repeated 4x, centred)
...
5. **All-caps headlines with Big Shoulders variable-width, tightly tracked.** `font-variation-settings: "wdth" 110, "wght" 800;`. Track them **tight**, not loose: `letter-spacing: -0.005em` on the hero word, `0.02em` on section heads. The loose `0.04em` look reads as AI-spread and is reserved **only** for the marquee banner. Multi-line heads use line-height `1.02` or higher. A single non-wrapping hero word may use `0.82` only with `recipe-exception: carnival-single-line-marquee` and rendered overflow proof.
...
h1, h2 { font-family: var(--font-display); font-weight: 800; font-variation-settings: "wdth" 110; letter-spacing: 0.02em; line-height: 1.02; text-transform: uppercase; }
.hero-word { letter-spacing: -0.005em; line-height: 0.82; white-space: nowrap; } /* recipe-exception: carnival-single-line-marquee */
