...
Load during [build workflow: Genre, Theme Route, and Diversification](../workflows/build.md#phase-3-genre-theme-route-and-diversification) whenever the catalog pick is `cobalt`. The OKLCH palette and font stack come from the source project's theme token catalog under `[data-theme="cobalt"]`. The upstream example is not included in this portable candidate.
