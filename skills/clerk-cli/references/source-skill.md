...
metadata:
  author: clerk
  version: 0.1
  skills_sh_url: "https://skills.sh/clerk/skills/clerk-cli"
  github_url: "https://github.com/clerk/skills/tree/HEAD/skills/core/clerk-cli"
---
...
`clerk init` does **not** require `clerk auth login`: an unauthenticated run mints an unclaimed app with temporary development keys — no account, no browser, no flag. Most instance commands then work on that key alone.
...
Which commands need an account, and the full rules: [auth.md](auth.md#accountless-operating-without-an-account).
...
A project is "linked" to an application via `clerk link`. Once linked, most commands auto-resolve the target app and dev instance from the repo's git remote. To target something else, pass `--app <id>` and/or `--instance dev|prod|<instance_id>`. See [references/auth.md](auth.md) for the full resolution order.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clerk init`                  | Scaffold Clerk into a project. Unauthenticated runs write temporary development keys by default — no login required (see [No login required](#no-login-required)). `--starter` only supports bootstrap for Next.js, React Router, Astro, Nuxt, TanStack Start, React, Vue, and JavaScript. `--keyless` with `--app`/`--login`, or `--template`/`--fresh` on any run that targets a real app (including while merely signed in), is a usage error — and it fires *after* scaffolding, so a starter run leaves the project behind. | `--framework`, `--pm`, `--name` (with `--starter`), `--app`, `--starter`, `--keyless`, `--login`, `--template`, `--fresh`, `-y`, `--no-skills`                                   |
| `clerk auth login`            | OAuth browser login (stores token). Not required for bootstrap or no-account instance config. Agent mode: no-op if already logged in. With no stored session it still opens a browser and binds a localhost callback, so it is not unattended; prefer `CLERK_PLATFORM_API_KEY` for headless flows. Aliases: `signup`, `signin`, `sign-in`. Top-level shortcut: `clerk login`. | -                                                                                                                                                                                |
...
Full matrix and sandbox details in [references/agent-mode.md](agent-mode.md).
...
- [references/auth.md](auth.md) - auth flow, key resolution order, host-vs-sandbox behavior, `--app`/`--instance` targeting, Backend vs Platform API.
- [references/recipes.md](recipes.md) - copy-pasteable recipes for common Clerk tasks.
- [references/agent-mode.md](agent-mode.md) - agent-mode behavior matrix, sandbox warning semantics, exit codes, error format.
