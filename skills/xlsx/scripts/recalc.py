...
def libreoffice_user_installation_arg(profile_dir):
...
def setup_libreoffice_macro(profile_dir):
    ...
    if (
        macro_file.exists()
        and "RecalculateAndSave" in macro_file.read_text()
    ):
        return True

    user_installation = libreoffice_user_installation_arg(profile_dir)

    if not macro_dir.exists():
        subprocess.run(
            ["soffice", user_installation, "--headless", "--terminate_after_init"],
            capture_output=True,
            timeout=10,
            env=get_soffice_env(),
        )
...
def recalc(filename, timeout=30):
    ...
    with tempfile.TemporaryDirectory(prefix="xlsx-lo-profile-") as profile_dir:
        ...
        if result.returncode != 0 and result.returncode != 124:
            ...
            if "Module1" in error_msg or "RecalculateAndSave" not in error_msg:
                return {"error": "Isolated LibreOffice macro not configured properly"}
