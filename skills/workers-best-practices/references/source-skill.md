...
| Workers types | See `review.md` for retrieval steps | API signatures, handler types, binding types |
...
- `rules.md` — all best practice rules with code examples and anti-patterns
- `review.md` — type validation, config validation, binding access patterns, review process
...
3. **Check types** — binding access, handler signatures, no `any`, no unsafe casts (see `review.md`)
...
8. **Reference rules** — see `rules.md` for each rule's correct pattern
