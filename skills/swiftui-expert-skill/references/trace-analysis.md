...
|------------------|---------------------------|-------------------------------------|
| `onChange`       | Expensive `.onChange` body | `performance-patterns.md`, `state-management.md` |
| `Gesture`        | Heavy gesture handler     | `performance-patterns.md` |
| `Action Callback`| Button/tap handler work   | `performance-patterns.md` |
| `Update`         | View body recomputation   | `view-structure.md`, `performance-patterns.md` |
| `Creation`       | View init cost            | `view-structure.md`      |
| `Layout`         | GeometryReader churn      | `layout-best-practices.md` |
...
- **`closure #1 in UserDefaultObserver.Target.GraphAttribute.send()`** —
  ...
  invalidate. Route to `state-management.md` and
...
- **`View Creation / Reuse`** as the #1 source — the hierarchy is
  ...
  `list-patterns.md` and `view-structure.md`.
