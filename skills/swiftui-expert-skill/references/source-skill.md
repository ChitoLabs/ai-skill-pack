...
metadata:
  ...
  github_url: "https://github.com/avdlee/swiftui-agent-skill/tree/HEAD/skills/swiftui-expert-skill"
---
...
- Consult `latest-apis.md` at the start of every task to avoid deprecated APIs
...
- Only adopt Liquid Glass when explicitly requested by the user (see `liquid-glass.md`)
...
- Flag deprecated APIs (compare against `latest-apis.md`)
- Run the Topic Router below for each relevant topic
- Validate `#available` gating and fallback paths for iOS 26+ features

### Improve existing SwiftUI code
- Audit current implementation against the Topic Router topics
- Replace deprecated APIs with modern equivalents from `latest-apis.md`
- Refactor hot paths to reduce unnecessary state updates
- Extract complex view bodies into separate subviews
- Suggest image downsampling when `UIImage(data:)` is encountered (optional optimization, see `image-optimization.md`)
...
Trigger when the user asks to "record a trace", "profile the app", "capture a session", etc. Full reference: `trace-recording.md`.
...
1. **Confirm target** — attach to a running app, launch an app, or record all processes? If the user didn't say, ask. List connected devices when useful:
   ...
   ```
2. **Pick a template based on target kind** — the `SwiftUI` template populates the SwiftUI lane on any **real device**: a physical iOS/iPadOS device **or the host Mac**. The only exception is the **iOS Simulator**, where the SwiftUI lane comes back empty — switch to `--template "Time Profiler"` in that case (still gives Time Profiler + Hangs + Animation Hitches). Always check `--list-devices`: `simulators` kind → `Time Profiler`; `devices` kind (real devices and the host Mac) → default `SwiftUI`. Full decision table in `trace-recording.md`.
...
Full reference: `trace-analysis.md`. Summary of the composition pattern:
...
3. **Run the main analysis** (with or without `--window`):
   ...
   ```
4. **Interpret with `trace-analysis.md`** — key diagnostics:
...
| View composition | `view-structure.md` |
| Performance | `performance-patterns.md` |
| Lists and ForEach | `list-patterns.md` |
| Layout | `layout-best-practices.md` |
| Sheets and navigation | `sheet-navigation-patterns.md` |
| ScrollView | `scroll-patterns.md` |
| Focus management | `focus-patterns.md` |
| Animations (basics) | `animation-basics.md` |
| Animations (transitions) | `animation-transitions.md` |
| Animations (advanced) | `animation-advanced.md` |
| Accessibility | `accessibility-patterns.md` |
| Swift Charts | `charts.md` |
| Charts accessibility | `charts-accessibility.md` |
| Image optimization | `image-optimization.md` |
| Liquid Glass (iOS 26+) | `liquid-glass.md` |
| macOS scenes | `macos-scenes.md` |
| macOS window styling | `macos-window-styling.md` |
| macOS views | `macos-views.md` |
| Text patterns | `text-patterns.md` |
| Deprecated API lookup | `latest-apis.md` |
| Instruments trace analysis | `trace-analysis.md` |
| Instruments trace recording | `trace-recording.md` |
...
- `latest-apis.md` -- **Read first for every task.** Deprecated-to-modern API transitions (iOS 15+ through iOS 26+)
- `state-management.md` -- Property wrappers, data flow, `@Observable` migration
- `view-structure.md` -- View extraction, container patterns, `@ViewBuilder`
- `performance-patterns.md` -- Hot-path optimization, update control, `_logChanges()`
- `list-patterns.md` -- ForEach identity, Table (iOS 16+), inline filtering pitfalls
- `layout-best-practices.md` -- Layout patterns, GeometryReader alternatives
- `accessibility-patterns.md` -- VoiceOver, Dynamic Type, grouping, traits
- `animation-basics.md` -- Implicit/explicit animations, timing, performance
- `animation-transitions.md` -- View transitions, `matchedGeometryEffect`, `Animatable`
- `animation-advanced.md` -- Phase/keyframe animations (iOS 17+), `@Animatable` macro (iOS 26+)
- `charts.md` -- Swift Charts marks, axes, selection, styling, Chart3D (iOS 26+)
- `charts-accessibility.md` -- Charts VoiceOver, Audio Graph, fallback strategies
- `sheet-navigation-patterns.md` -- Sheets, NavigationSplitView, Inspector
- `scroll-patterns.md` -- ScrollViewReader, programmatic scrolling
- `focus-patterns.md` -- Focus state, focusable views, focused values, default focus, common pitfalls
- `image-optimization.md` -- AsyncImage, downsampling, caching
- `liquid-glass.md` -- iOS 26+ Liquid Glass effects and fallback patterns
- `macos-scenes.md` -- Settings, MenuBarExtra, WindowGroup, multi-window
- `macos-window-styling.md` -- Toolbar styles, window sizing, Commands
- `macos-views.md` -- HSplitView, Table, PasteButton, AppKit interop
- `text-patterns.md` -- Text initializer selection, verbatim vs localized
- `trace-analysis.md` -- Parse Instruments `.trace` files via `../scripts/analyze_trace.py`; interpret main-thread coverage, high-severity SwiftUI updates, hitch narratives, and map findings back to source files
- `trace-recording.md` -- Record a new trace via `../scripts/record_trace.py`: attach to a running app, launch one fresh, or capture a manually-stopped session; supports stop-file for agent-driven flows
