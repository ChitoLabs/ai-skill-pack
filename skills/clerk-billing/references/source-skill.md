...
|-----------|-------------|
| `billing-components.md` | `<PricingTable />` and subscription UI |
| `b2c-patterns.md` | B2C subscription billing patterns |
| `b2b-patterns.md` | B2B billing with organization subscriptions and seat-limit plans |
| `billing-webhooks.md` | Subscription lifecycle event handling |
...
See `b2b-patterns.md` for tiered plan naming, seat-limit invariants, admin-only billing, and webhook handlers.
...
For the full template covering all 15 events, the TS type declarations from `@clerk/backend`, the `proxy.ts` public-route setup, and the subscription status value table, see `billing-webhooks.md`.
